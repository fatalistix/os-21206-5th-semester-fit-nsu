...
queue_t *queue_init(int max_count) {
  ...
  err = sem_init(&q->sem_modify, 0, 1);
  if (err) {
    printf("queue_init: sem_init() failed: %s\n", strerror(err));
    abort();
  }

  err = sem_init(&q->sem_add, 0, max_count);
  if (err) {
    printf("queue_init: sem_init() failed: %s\n", strerror(err));
    abort();
  }

  err = sem_init(&q->sem_get, 0, max_count);
  if (err) {
    printf("queue_init: sem_init() failed: %s\n", strerror(err));
    abort();
  }
  ...
  for (int i = 0; i < max_count; ++i) {
    err = sem_wait(&q->sem_get);
    if (err) {
      printf("queue_init: sem_wait() failed: %s\n", strerror(err));
      abort();
    }
  }
...
void queue_destroy(queue_t *q) {
  ...
  err = sem_destroy(&(q->sem_modify));
  if (err) {
    printf("queue_destroy: sem_destroy() failed: %s\n", strerror(err));
    abort();
  }

  err = sem_destroy(&(q->sem_add));
  if (err) {
    printf("queue_destroy: sem_destroy() failed: %s\n", strerror(err));
    abort();
  }

  err = sem_destroy(&(q->sem_get));
  if (err) {
    printf("queue_destroy: sem_destroy() failed: %s\n", strerror(err));
    abort();
  }
...
int queue_add(queue_t *q, int val) {
  ...
  err = sem_post(&q->sem_modify);
  if (err) {
    printf("queue_add: sem_get() failed: %s\n", strerror(err));
    abort();
  }

  q->add_count++;

  err = sem_post(&(q->sem_get));
  if (err) {
    printf("queue_add: sem_post() failed: %s\n", strerror(err));
    abort();
  }
