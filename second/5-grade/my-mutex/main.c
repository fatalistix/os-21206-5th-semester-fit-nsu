...
int lock_init(node_t *node) { return my_mutex_init(&(node->sync)); }

int destroy_lock(node_t *node) { return 0; }

int lock_node(node_t *node) { return my_mutex_lock(&(node->sync)); }

int unlock_node(node_t *node) { return my_mutex_unlock(&(node->sync)); }
