...
int destroy_lock(node_t *node) { return pthread_spin_destroy(&(node->sync)); }

int lock_node(node_t *node) { return pthread_spin_lock(&(node->sync)); }

int unlock_node(node_t *node) { return pthread_spin_unlock(&(node->sync)); }
