...
queue_t *queue_init(int max_count) {
  ...
  if (err) {
    printf("Cannot init add condition for a queue\n");
    abort();
  }

  err = pthread_cond_init(&(q->get_cond), NULL);
  if (err) {
    printf("Cannot init get condition for a queue\n");
    abort();
  }
...
void queue_destroy(queue_t *q) {
  ...
  err = pthread_cond_destroy(&(q->add_cond));
  if (err) {
    printf("queue_destroy: pthread_cond_destroy() failed: %s\n", strerror(err));
    abort();
  }

  err = pthread_cond_destroy(&(q->get_cond));
  if (err) {
    printf("queue_destroy: pthread_cond_destroy() failed: %s\n", strerror(err));
    abort();
  }
