...
int lock_init(node_t *node) { return pthread_mutex_init(&(node->sync), NULL); }

int destroy_lock(node_t *node) { return pthread_mutex_destroy(&(node->sync)); }

int lock_node(node_t *node) { return pthread_mutex_lock(&(node->sync)); }

int unlock_node(node_t *node) { return pthread_mutex_unlock(&(node->sync)); }
